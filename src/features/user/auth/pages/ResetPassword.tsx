// src/features/user/auth/pages/ResetPassword.tsx
  ...
  return (
    <Box sx={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: theme.palette.background.default, px: 2 }}>
      <Paper elevation={6} sx={{ width: '100%', maxWidth: 420, p: { xs: 4, sm: 5 }, borderRadius: 2, textAlign: 'center' }}>
        <Typography variant="h5" fontWeight={700} gutterBottom sx={{ mb: 1 }}>Reset Password</Typography>
        <Typography variant="body2" sx={{ mb: 4, color: theme.palette.text.secondary }}>Enter your new password below.</Typography>
        ...
        <Box component="form" onSubmit={handleSubmit(handleSubmitForm)} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          <TextField label="Email Address" type="email" fullWidth variant="outlined" error={!!errors.email} helperText={errors.email?.message} {...register('email')} disabled />
          <TextField label="Reset Code" type="text" fullWidth variant="outlined" error={!!errors.resetCode} helperText={errors.resetCode?.message} {...register('resetCode')} disabled />
          <TextField label="New Password" type="password" fullWidth variant="outlined" error={!!errors.newPassword} helperText={errors.newPassword?.message} {...register('newPassword')} />
          <TextField label="Confirm Password" type="password" fullWidth variant="outlined" error={!!errors.passwordConfirm} helperText={errors.passwordConfirm?.message} {...register('passwordConfirm')} />
